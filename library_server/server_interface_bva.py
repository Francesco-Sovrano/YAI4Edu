...
cache_path = os_path.join(base_path,'cache','bva')
# cache_path = os_path.join(base_path,'cache','bva','small')
document_path = os_path.join(base_path,'documents','bva')
...
from knowpy.misc.doc_reader import load_or_create_cache, load_cache, DocParser, get_document_list, get_content_list
...
QA_CLEANING_OPTIONS = {
	...
	'min_question_to_sentence_overlap': 0.7,
...
qa_cache = os_path.join(cache_path,'adaptive_qa_embedder.pkl' if ADAPTIVE_QA else 'qa_embedder.pkl')
...
def init(model_family, information_units):
	...
	# 	old_graph = load_cache(os_path.join(cache_path,f"_old_graph_clauses_lemma-False.pkl"))
	# 	old_kg_manager = KnowledgeGraphManager(KG_MANAGER_OPTIONS, old_graph)
	# 	del old_graph
	# 	doc_uri_dict = old_kg_manager.adjacency_list.get_predicate_dict(DOC_ID_PREDICATE)
	# 	content_list = [
	# 		{
	# 			'text': content,
	# 			'id': DOC_PREFIX+get_uri_from_txt(doc_uri.replace('myfile:','')+'.txt')
	# 		}
	# 		for source_uri,content_list in old_kg_manager.content_dict.items()
	# 		for doc_uri in doc_uri_dict[source_uri]
	# 		for content in content_list
	# 	]
	# 	print(f'Processing {len(content_list)} documents')
	# 	del old_kg_manager
	# 	chunks = tuple(get_chunks(content_list, elements_per_chunk=DOCUMENTS_PER_CHUNK))
	# 	num_chunks = math.ceil(len(content_list)/DOCUMENTS_PER_CHUNK)
	# 	del content_list
	# 	kg_builder = KnowledgeGraphBuilder(GRAPH_BUILDER_OPTIONS)
	# 	for i,docs in enumerate(tqdm(chunks)):
	# 		load_or_create_cache(graph_cache+f'.{i}_{num_chunks}_{normal_graph_without_jumps}.pkl', lambda: kg_builder.set_content_list(docs, avoid_jumps=normal_graph_without_jumps).build(add_verbs=False, add_predicates_label=False))
	# 	# Add file mapping
	# 	document_list = get_document_list(document_path)
	# 	document_list = [d for d in document_list if d.endswith('json')]
	# 	graph = flatten(
	# 		(
	# 			c['graph']
	# 			for c in get_content_list(document_list)
	# 			if 'graph' in c
	# 		), 
	# 		as_list=True
	# 	)
	# 	for i,docs in enumerate(tqdm(chunks)):
	# 		graph += load_or_create_cache(graph_cache+f'.{i}_{num_chunks}_{normal_graph_without_jumps}.pkl', lambda: kg_builder.set_content_list(docs, avoid_jumps=normal_graph_without_jumps).build(add_verbs=False, add_predicates_label=False))
	# 		# graph += load_or_create_cache(graph_cache+f'.{i}_{num_chunks}_{normal_graph_without_jumps}_{get_iter_uid(docs)}.pkl', lambda: kg_builder.set_document_list(docs, avoid_jumps=normal_graph_without_jumps).build(add_verbs=False, add_predicates_label=False))
	# 		graph = list(unique_everseen(graph))
	# 	return graph

	def extract_graph():
		print('Building Graph..')
		content_list = get_content_list(get_document_list(document_path))
		print(f'Processing {len(content_list)} documents')
		chunks = tuple(get_chunks(content_list, elements_per_chunk=PARAGRAPHS_PER_CHUNK))
		num_chunks = math.ceil(len(content_list)/PARAGRAPHS_PER_CHUNK)
		del content_list
		kg_builder = KnowledgeGraphBuilder(GRAPH_BUILDER_OPTIONS)
		for i,docs in enumerate(tqdm(chunks)):
			load_or_create_cache(graph_cache+f'.{i}_{num_chunks}_{normal_graph_without_jumps}.pkl', lambda: kg_builder.set_content_list(docs, avoid_jumps=normal_graph_without_jumps).build(add_verbs=False, add_predicates_label=False))
	...
	# )
	# save_graphml(graph, 'knowledge_graph_bva')
