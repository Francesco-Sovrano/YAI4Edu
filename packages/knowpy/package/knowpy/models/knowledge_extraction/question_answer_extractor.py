...
class QuestionAnswerExtractor(ModelManager):
	...
	def extract_aligned_graph_from_qa_dict_list(self, graph, graph_builder_options, qa_dict_list=None, qa_type_to_use=None, cleaning_options=None, elements_per_chunk=None, avoid_jumps=False, parallel_extraction=True, cache_path=None, add_verbs=False, add_predicates_label=False, use_paragraph_text=False):
		# Build qa_list
		if qa_dict_list is None:
			qa_dict_list = self.extract(graph)
		if cleaning_options:
			qa_dict_list = self.clean_qa_dict_list(qa_dict_list, **cleaning_options)
		# if qa_type_to_use is None:
		# 	qa_type_to_use= [
		...
		self.logger.info(f'QuestionAnswerExtractor::extract_aligned_graph_from_qa_dict_list - Building AdjacencyList..')
		original_adjacency_list = AdjacencyList(
			graph, 
			equivalence_relation_set=set([IS_EQUIVALENT_PREDICATE]),
			is_sorted=True,
		)
		self.logger.info(f'QuestionAnswerExtractor::extract_aligned_graph_from_qa_dict_list - AdjacencyList built')

		# Set correct paragraph_text and doc before building the EDUs graph
		span_source_id_dict = original_adjacency_list.get_predicate_dict(HAS_SOURCE_ID_PREDICATE)
		doc_uri_dict = original_adjacency_list.get_predicate_dict(DOC_ID_PREDICATE)
		source_uri_dict = original_adjacency_list.get_predicate_dict(HAS_PARAGRAPH_ID_PREDICATE)
		content_dict = original_adjacency_list.get_predicate_dict(HAS_CONTENT_PREDICATE)
		if use_paragraph_text:
			source_to_sentence_dict = original_adjacency_list.get_predicate_dict(HAS_CONTENT_PREDICATE, singlefy)
		else:
			source_to_sentence_dict_iter = original_adjacency_list.get_predicate_dict(HAS_SOURCE_LABEL_PREDICATE, singlefy).items()
			source_to_sentence_dict_iter = filter(lambda x: x[0] not in span_source_id_dict, source_to_sentence_dict_iter) # avoid clauses, consider sentences only
			source_to_sentence_dict_iter = map(lambda x: (source_uri_dict[x[0]][0], x[1]), source_to_sentence_dict_iter)
			source_to_sentence_dict = dict(source_to_sentence_dict_iter)
		### Clean memory
		del original_adjacency_list
		################
		...
		for qa_dict in qa_dict_list:
			...
			if not source_uri:
				...
				continue
			question_answer = qa_dict['abstract'] # QA extractor is not normalising strings, but KnowledgeGraphBuilder will
		...
		def process_chunk(contents):
			...
			for subj_dict, predicate_dict, obj_dict in kg_builder.triplet_tuple:
				...
				for sentence, source_uri in qa2sentence_dict[qa]:
					new_predicate_dict = dict(predicate_dict) # copy
					new_predicate_dict['source'] = dict(predicate_dict['source']) # copy

					new_predicate_dict['source']['sentence_text'] = sentence
					# assert len(content_dict[source_uri]) == 1
					new_predicate_dict['source']['paragraph_text'] = max(content_dict[source_uri],key=len)
					new_predicate_dict['source']['doc'] = doc_uri_dict[source_uri][0]#.replace(DOC_PREFIX,'') # Do not remove DOC_PREFIX here
					new_predicate_dict['source']['annotation'] = {
						...
						'content': [], # no need to add extra content, it's already in the 'source graph'
					}
			...
			edge_list = kg_builder.get_edge_list(
		...
		edu_graph = tuplefy(edu_graph)

		# Add source_id sub-graphs, taxonomical relations and other annotations
		invalid_uri = get_object_set(filter(lambda x: x[1] == HAS_SPAN_ID_PREDICATE, graph))
		useful_subgraph = filter(lambda x: x[1] != HAS_SPAN_ID_PREDICATE, graph)
		useful_subgraph = filter(lambda x: x[1] != HAS_SOURCE_ID_PREDICATE, useful_subgraph)
		useful_subgraph = filter(lambda x: x[1] != HAS_VERB_PREDICATE, useful_subgraph)
		useful_subgraph = filter(lambda x: x[1] != HAS_CONTENT_PREDICATE, useful_subgraph)
		useful_subgraph = filter(lambda x: x[0] not in invalid_uri, useful_subgraph)
		
		useful_subgraph = filter(lambda x: '{obj}' not in x[0], useful_subgraph)
		useful_subgraph = filter(lambda x: '{obj}' not in x[1], useful_subgraph)
		useful_subgraph = filter(lambda x: '{obj}' not in x[-1], useful_subgraph)
		edu_graph += useful_subgraph
	...
	def clean_qa_dict_list(self, qa_dict_list, sorted_template_list=None, min_qa_pertinence=0.05, max_qa_similarity=0.9502, max_answer_to_question_overlap=0.75, min_answer_to_sentence_overlap=0.75, min_question_to_sentence_overlap=0.75, coreference_resolution=False, cache_path=None):
		################
		# Remove QAs whose answer is not in the sentence
		def qa_in_sentence_ratio(qa, sentence):
			qa_set = set(qa.casefold().strip('.').split(' '))
			qa_in_sentence_set = qa_set.intersection(sentence.casefold().strip('.').split(' '))
			return len(qa_in_sentence_set)/len(qa_set)
		...
		################
		# # Remove QAs whose answer is too similar to the question
		# if max_answer_to_question_similarity < 1:
		# 	self.logger.info('QuestionAnswerExtractor::clean_qa_dict_list - Remove QAs whose answer is too similar to the question')
		# 	answer_to_question_similarity_fn = lambda: self.get_element_wise_similarity(question_list, answer_list, source_without_context=False, target_without_context=False)
		# 	answer_to_question_similarity_list = load_or_create_cache(
		# 		cache_path+f'.answer_to_question_similarity_list.{get_iter_uid(question_list+answer_list)}.pkl', 
		# 		answer_to_question_similarity_fn
		# 	) if cache_path else answer_to_question_similarity_fn()
		# 	question_list = [x for i,x in enumerate(question_list) if answer_to_question_similarity_list[i] <= max_answer_to_question_similarity]
		# 	answer_list = [x for i,x in enumerate(answer_list) if answer_to_question_similarity_list[i] <= max_answer_to_question_similarity]
		# 	abstract_list = [x for i,x in enumerate(abstract_list) if answer_to_question_similarity_list[i] <= max_answer_to_question_similarity]
		# 	qa_dict_list = [
		# 		x 
		# 		for i,x in enumerate(qa_dict_list) 
		# 		if answer_to_question_similarity_list[i] <= max_answer_to_question_similarity
		# 	]		
		# 	assert len(question_list)==len(answer_list)==len(abstract_list)==len(qa_dict_list), f'len(question_list) {len(question_list)} == len(answer_list) {len(answer_list)} == len(abstract_list) {len(abstract_list)} == len(qa_dict_list) {len(qa_dict_list)}'
		...
		if min_qa_pertinence > 0:
			self.logger.info('QuestionAnswerExtractor::clean_qa_dict_list - Remove QA whose answers are deemed to be not related to the question')
			qa_dict_list = [
				x 
				for i,x in enumerate(qa_dict_list) 
			...
			qa_dict_list += [
				{
					...
					'sentence': qa_dict[EXPLANATORY_TEMPLATE_PREDICATE],
