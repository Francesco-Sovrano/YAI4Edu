...
//------------ Dataset ------------
	...
	},
	{
		...
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			var [label,value] = value_list;
			return get_formatted_tuple_template(is_in_array, value, 
				x=>`There is 1 <b>mention</b> of ${label}: ${x}`,
				x=>`There are ${x.length} different <b>mentions</b> of ${label}`,
			)
		}
	},
	{ // derivations
...
//------------ YAI4Law ------------
	...
	// },
	{
		'keys': ['my:content', 'my:docID', 'my:hasIDX', 'my:article_id', 'my:paragraph_id', 'my:block_id', 'my:section_id', 'my:chapter_id', 'my:recital_id', 'my:reference_id', 'my:page_id'],
		'optional_keys': ['my:hasIDX', 'my:article_id', 'my:paragraph_id', 'my:block_id', 'my:section_id', 'my:chapter_id', 'my:recital_id', 'my:reference_id', 'my:page_id'],
		'hide_descendants': true,
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			var [content,doc_url,label_idx,article_id,paragraph_id,block_id,section_id,chapter_id,recital_id,reference_id,page_id] = value_list;
			var head_list = [];
			var value_list = [];
			// Get article
			var article = [];
			if (recital_id)
				article.push(get_description(recital_id));
			if (article_id)
			...
			var page = [];
			if (page_id)
				page.push(get_description(page_id))
			// Populate table
			if (article.length > 0)
			{
				head_list.push(recital_id?'Recital':'Article');
				value_list.push(article.join('.'));
			}
			if (section.length > 0)
			{
				head_list.push(chapter_id?'Chapter':'Section');
				value_list.push(section.join('.'));
			}
			if (page.length > 0)
			{
				head_list.push('Page');
				value_list.push(page.join('.'));
			}
			...
			head_list = head_list.concat(['Source']);
			value_list = value_list.concat([linkify(doc_url, get_known_label(doc_url))]);
	...
	{ 
		'keys': ['my:triple','my:abstract','my:confidence','my:sentence','my:hasSource','my:source_id','my:syntactic_similarity','my:semantic_similarity', 'my:relevant_to', 'my:score', 'my:question_pertinence_set'],
		'optional_keys': ['my:triple','my:abstract','my:hasSource','my:source_id','my:syntactic_similarity','my:semantic_similarity', 'my:relevant_to', 'my:score', 'my:question_pertinence_set'],
		'hide_descendants': true,
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			var [triple,abstract,pertinence,sentence,source,source_id,syntactic_similarity,semantic_similarity, relevant_to, score, question_pertinence_set] = value_list;
			const p = prefixed_string_to_uri;
			var head_list = ['Pertinence'];
			var value_list = [format_percentage(pertinence)];
			if (source)
			{
				// console.log(source, source[p('my:article_id')], p('my:article_id'));
				const doc_url = get_description(source[p('my:docID')], false);
				// Populate table
				// if (abstract.length > 0)
				// {
				// 	head_list.push('Pointer');
				// 	value_list.push(abstract);
				// }
				///////
				var source_details = source[p('my:paraID')];
				if (source_details)
				{
					if (isArray(source_details)) {
						for (var [key,value] of Object.entries(source_details))
						{
							if (isDict(value))
				...
				const article_id = get_description(source_details[p('my:article_id')]);
				const paragraph_id = get_description(source_details[p('my:paragraph_id')]);
				const block_id = get_description(source_details[p('my:block_id')]);
				const chapter_id = get_description(source_details[p('my:chapter_id')]);
				const section_id = get_description(source_details[p('my:section_id')]);
				const recital_id = get_description(source_details[p('my:recital_id')]);
				const page_id = get_description(source_details[p('my:page_id')]);
				// Get section
				var section = [];
				if (chapter_id)
					section.push(get_description(chapter_id));
	...
	},
	{
		...
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			...
			if (quality_percentage < 0.3)
				...
				quality_value = 'Good';
			return `<span class="answer_quality_${quality_value.toLowerCase().replace(' ','_')}">The <b>quality</b> of this answer is estimated to be <b>${quality_value}</b></span>`;
		}
	},
	{
