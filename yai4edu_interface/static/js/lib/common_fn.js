...
function template_expand(name,topic=null,label=null) 
{
	...
	return `<annotation><span 
					class="link"
					data-topic="${topic}"
					data-label="${label.length>name.length?label:name}"
				>${name}<span class="tooltiptext">Click to overview</span></span></annotation>`;
	// return `<annotation><span 
	// 				class="link"
	// 				data-topic="${topic}"
	// 				data-label="${label}"
...
function htmlDecode(input){
  ...
  return e.childNodes.length === 0 ? "" : Array.from(e.childNodes).map(x=>x.nodeValue).join('');
}
...
// const remove_presentation = y=>y.replace(/<span role="presentation"[^>]*>([^<]*)<\/span>/g,'$1');
const remove_hyphenation = y=>y.replace(/([^ ])-[\n ]+/g,'$1');
const remove_ending_hyphenation = y=>y.replace(/([^ ])-[\n ]*$/g,'$1');
...
function annotate_html(html, inner_text, annotation_list, annotate_fn, merge_span=false)
{
	...
	inner_text = tokenise(remove_hyphenation(inner_text).replace(/\n/g,'').trim()).join(' ').replace(/ +/g,' ');
	// console.log(inner_text);
	// console.log(JSON.stringify(annotation_list, null, 4));
	var annotated_html = remove_orphans(htmlDecode(html).replace(/\n/g,'').trim()); // get decoded html
	...
	for (const annotation_dict of annotation_list)
	{
		...
		while (start_idx >= 0)
		{
			...
			for (var i=splitted_html.length-1; !added && i>=0; --i)
			{
				...
				const label_is_in_content = splitted_content.join(' ').search(regexp) >= 0;
				...
				if (!label_is_in_content && merge_span) {
					...
					// Proceed backward
					{
						...
						while (!stop_iterating && j >= 0) {
							...
							// 	continue;
							const new_content = jth_html_piece.startsWith('<br')?' ':jth_html_piece.split('<')[0];
							// if (content.includes('gov') || content.includes('rnment'))
							// 	console.log('--j', content, '@', new_content, '@', jth_html_piece);
							left_proxy_content = new_content+left_proxy_content;
							if (new_content != ' ' && new_content.includes(' '))
								stop_iterating = true;
						}
					}
					// Proceed forward
					...
					left_proxy_content = remove_ending_hyphenation(left_proxy_content);
					right_proxy_content = remove_ending_hyphenation(right_proxy_content);
				}
				// const proxy_content = left_proxy_content+content+right_proxy_content;

				const split_left_proxy_content = tokenise(left_proxy_content);
				const split_right_proxy_content = tokenise(right_proxy_content);
				const left_missing_content = split_left_proxy_content.join(' ');
				const right_missing_content = split_right_proxy_content.join(' ');

				const joined_tokenised_proxy_content = remove_hyphenation(left_missing_content + content + right_missing_content);
				if (!label_is_in_content && joined_tokenised_proxy_content.search(regexp) < 0)
					continue;
				// if (annotation_uri=='my:state_government')
				// {
				// 	console.log('left_missing_content:', left_missing_content);
				// 	console.log('right_missing_content:', right_missing_content);
				// 	console.log('splitted_content:', splitted_content);
				// 	console.log('joined_tokenised_proxy_content:', joined_tokenised_proxy_content);
				// }
				...
				for (var s=splitted_content.length-1; !added && s>=0; --s)
				{
					...
					if (!found && !label_is_short)
					{
						...
						{
							var content_portion = splitted_content.slice(s,last_s+1).join(' ').trim();
							if (content_portion == '')
								continue;
							...
							{
								// Merge broken tokens
								var considered_left_missing_content = left_missing_content.trim();
								if (!left_missing_content.endsWith(' '))
								{
									const last_left_proxy_content = split_left_proxy_content[split_left_proxy_content.length-1];
									content_portion = last_left_proxy_content+content_portion;
									considered_left_missing_content = considered_left_missing_content.slice(0,considered_left_missing_content.length-last_left_proxy_content.length).trim();
								}
								var considered_right_missing_content = right_missing_content.trim();
								if (!right_missing_content.startsWith(' '))
								{
									const first_right_proxy_content = split_right_proxy_content[0];
									content_portion = content_portion+first_right_proxy_content;
									considered_right_missing_content = considered_right_missing_content.slice(first_right_proxy_content.length,considered_right_missing_content.length).trim();
								}
								// Understand if current token (content_portion) is part of a broken syntagma annotation
								...
								if (annotation_label_ends_with_content_portion)
								{
									...
									cannot_start = considered_left_missing_content.endsWith(' '+missing_beginning_of_content) || (considered_left_missing_content.length==missing_beginning_of_content.length && considered_left_missing_content.endsWith(missing_beginning_of_content));
								}
								...
								if (annotation_label_starts_with_content_portion)
								{
									...
									cannot_end = considered_right_missing_content.startsWith(missing_end_of_content+' ') || (considered_right_missing_content.length==missing_end_of_content.length && considered_right_missing_content.startsWith(missing_end_of_content));
								}
								...
								// 	console.log('		considered_left_missing_content:', considered_left_missing_content)
								// 	console.log('		considered_right_missing_content:', considered_right_missing_content)
								// 	console.log('		left_missing_content:', left_missing_content)
								// 	console.log('		right_missing_content:', right_missing_content)
